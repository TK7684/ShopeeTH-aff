import { fetchAllPages, filterProducts } from "~/server/utils/fetchProducts";
import { uploadToGoogleSheet } from "~/server/utils/googleSheets";
import {
  buildSheetRows,
  buildCategoryRows,
  buildCategoryTabs,
} from "~/server/utils/sheetBuilders";

// Configuration from Environment Variables
const SHEET_ID =
  process.env.SHEET_ID ||
  process.env.GOOGLE_SHEETS_ID ||
  "1l3PESURRNXpZDTxQbZYM2KxpdPvWcZdnGVKQC_jKtFo";
const SHEET_RANGE = process.env.GOOGLE_SHEETS_RANGE || "DailyTop!A1";
const CATEGORY_RANGE =
  process.env.GOOGLE_SHEETS_CATEGORY_RANGE || "CategoryTop!A1";
...
const HISTORY_LIMIT = parseInt(
  process.env.PIPELINE_HISTORY_LIMIT || "1000",
  10,
);
const CATEGORY_TOP_LIMIT = parseInt(
  process.env.PIPELINE_CATEGORY_TOP_LIMIT || "5",
  10,
);
const CATEGORY_TAB_LIMIT = parseInt(
  process.env.PIPELINE_CATEGORY_TAB_LIMIT || "20",
  10,
);
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const categoryTabs = buildCategoryTabs(
      filteredProducts,
      CATEGORY_TAB_LIMIT,
    );
