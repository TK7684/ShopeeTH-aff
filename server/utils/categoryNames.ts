...
const MAIN_CATEGORY_MAP: Record<number, string> = {
    // Food & Beverages
...
export function getCategoryName(id: any): string | null {
    const key = normalizeId(id);
    if (!key) return null;
    if (Object.prototype.hasOwnProperty.call(MAIN_CATEGORY_MAP, key)) {
        return MAIN_CATEGORY_MAP[Number(key)];
    }
    return `หมวดหมู่ไม่ทราบ (${key})`;
}

export function getCategoryNameList(ids: any[]): string[] {
...
export function getMainCategoryId(categoryIds: any[]): string | null {
    if (!Array.isArray(categoryIds) || categoryIds.length === 0) return null;
    const first = normalizeId(categoryIds[0]);
    return first || null;
}

export function getMainCategorySheetTitle(categoryId: any): string {
