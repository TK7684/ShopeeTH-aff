...
if (options.help) {
  ...
  console.log("  --ui               Run tests with UI interface");
  ...
  console.log(
    "  node scripts/run-tests.js --api-only         # Run only API tests",
  );
  console.log(
  ...
  console.log(
    "  node scripts/run-tests.js --ui               # Run tests with UI interface",
  );
  process.exit(0);
}
...
// Run the tests
console.log(
  "Running tests with command: npm run test:run " + vitestArgs.join(" "),
);
